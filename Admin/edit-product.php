...
<?php 
if(isset($_GET['upproductid'])){
    $updatepid=$_GET['upproductid'];
...
//Query start
        if(!empty($product) && !empty($desc) && !empty($price) && !empty($stock)){
            if(!preg_match("/^[a-zA-Z0-9-' ]*$/",$product)){
                ...
                $update=mysqli_query($conn,"UPDATE product SET product_name='$product',product_desc='$desc',price='$price',stock='$stock',file='$file',date=NOW(),sub_cat_id='$category' WHERE product_id=$updatepid");
                if($update){
                    echo $_SESSION['done-msg'] ="<div class='alert alert-success'>Product successful Updated.</div>";
                header("location:all-product.php");
                exit();
                }else{
                    echo $_SESSION['done-msg'] = "<div class='alert alert-danger'>Query error.</div>";
                header("location:edit-product.php?error-message=1");
                exit();
                }
            }
        }else{
            ...
            <div>
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input type="text" name="product" class="form-control" value="<?php if(isset($product)) { echo $product;}?>">
                    </div>
                    <div class="form-group">
                        <label for="desc" class="control">Product Description</label>
                        <input type="text" name="desc" class="form-control" value="<?php if(isset($description)) { echo $description;}?>">
                    </div>
                    <div class="form-group">
